...
<div class="max-w-xl mx-auto py-8">
    ...
    <form action="{{ route('ventas.propuestas.efectuar', $propuesta->idSalesProposals) }}" method="POST" class="bg-white rounded shadow p-6 space-y-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="number" name="UnitPrice" class="w-full rounded border-gray-300" min="0" step="0.01" required>
        </div>
        <div class="flex justify-end space-x-2">
            <a href="{{ route('ventas.propuestas') }}" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Cancelar</a>
            <button type="submit" class="px-4 py-2 bg-brand-blue text-white rounded hover:bg-blue-700">Confirmar Venta</button>
        </div>
    </form>
</div>
@endsection